...
function processContains(item,list,callback) {
 return callback(list.indexOf(item) === -1 ? false : true)
...
function processDuplicateFree(list ,cb) {
  /* CODE HERE ONLY AFTER COMPLETING ALL OTHER TASK*/
}
...
function firstNamesAllCaps(runners) {
  return runners =  runners.map((runner) =>{
...
function tallyUpDonations(runners) {
  return runners.reduce((totalDonation,individualDonation) => {
  return totalDonation + individualDonation.donation;
}, 0);
}
/////////////// CLOSURES ///////////////
/////////////// CLOSURES ///////////////
...
function counterMakerWithLimit(limit) {
 ...
 const newcounter = counterMakerWithLimit(3)
